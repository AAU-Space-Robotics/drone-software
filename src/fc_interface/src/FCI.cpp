...
void OffboardControl::publish_bodyrate_setpoint()
{
    VehicleRatesSetpoint msg{};
    msg.timestamp = this->get_clock()->now().nanoseconds() / 1000;
    msg.roll = 10.0;   // [rad/s] roll rate setpoint
    msg.pitch = 10.0;  // [rad/s] pitch rate setpoint
    msg.yaw = 10.0;    // [rad/s] yaw rate setpoint
    msg.thrust_body = {0.0, 0.0, -0.5}; // Normalized thrust command in body NED frame [-1,1]
    msg.reset_integral = false; // Reset roll/pitch/yaw integrals
